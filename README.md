...
**内閣府の公式 CSV から日本の祝日を取得する、薄い Node.js ライブラリ + CLI。** ハードコードしない、ライブラリ更新を待たない、データは常に最新。
...
既存の祝日ライブラリは大きく 2 系統あり、それぞれ弱点がある:

- **静的データ埋め込み式**（`@holiday-jp/holiday_jp` など最有名どころ）: 翌年の祝日や臨時祝日が入るのはパッケージ更新後
- **ルール計算式**（`japanese-holidays` ほか複数）: 祝日法改正や即位の礼のような臨時祝日に対応できない
...
**アンチパターン**: リクエストやイベント単位で fetch 系関数を呼ぶ。毎回ネットワーク往復になり、内閣府サーバにも負荷をかけます。取得は1回、判定は自前ストアから、が基本です。
